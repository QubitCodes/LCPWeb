        ...
        <div className="flex flex-col h-full bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transition-colors duration-200">
            ...
            <div className="flex-1 overflow-y-auto py-4 custom-scrollbar">
                ...
                <nav className="space-y-0.5">
                    {/* Dashboard Redirects */}
                    <NavItem
                        href={isAdmin ? '/admin/dashboard' : isSupervisor ? '/admin/supervisor' : '/admin/worker'}
                    ...
                    {isAdmin && (
                        <>
                            <NavItem href="/admin/users" icon={Users} label="Admins" />
                            {/*
                    ...
                    {/* Shared Links */}
                    {role === 'SUPER_ADMIN' && <NavItem href="/admin/enrollments" icon={CreditCard} label="Enrollments" />}

                    {isAdmin && <NavItem href="/admin/audit-logs" icon={History} label="Audit Logs" />}
                </nav>
            </div>
